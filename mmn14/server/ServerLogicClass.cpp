 ...
 */
std::string ServerLogicClass::randString(const uint32_t length) const
{
	if (length == 0)
		return "";
	auto randChar = []() -> char
 ...
 */
bool ServerLogicClass::userHasFiles(const uint32_t userId)
{
	if (userId == 0)
		return false;
	std::stringstream ss;
...
/**
   @brief Parses a given filename as a bytes array.
   ...
   @return true if filename was parsed successfully, false otherwise.
 */
bool ServerLogicClass::parseFilename(const uint16_t filenameLength, const uint8_t* filename, std::string& parsedFilename)
{
...
/***
   @brief Copies a filename from request to response. the calling function is responsible for freeing the allocated memory.
   @param request The source of the filename
   @param response The destination for the filename copy
 */
void ServerLogicClass::copyFilename(const SRequest& request, SResponse& response)
{
...
/**
   ...
   @param err Description error string stream.
   @return true if operation succeeded, false otherwise.
 */
bool ServerLogicClass::handleSocketFromThread(boost::asio::ip::tcp::socket& sock, std::stringstream& err)
{
	try
	{
		uint8_t buffer[PACKET_SIZE];
		SRequest* request = nullptr;
		SResponse* response = nullptr;
		bool responseSent = false;

		if (!_socketHandler.receive(sock, buffer))
		{
			err << "ServerLogicClass::handleSocketFromThread: Failed to receive first message from socket!" << std::endl;
			return false;
		}
...
/**
   @brief Handles a client's request. While SResponse is allocated in this function,
 ...
 */
bool ServerLogicClass::handleRequest(const SRequest& request, SResponse*& response, bool& responseSent, boost::asio::ip::tcp::socket& sock, std::stringstream& err)
{
	responseSent = false;
...
/**
   @brief Deserializes raw data into SRequest data format.
 */
ServerLogicClass::SRequest* ServerLogicClass::deserializeRequest(const uint8_t* const buffer, const uint32_t size)
{
...
/**
   @brief Serializes SResponse data format into the buffer.
 */
void ServerLogicClass::serializeResponse(const SResponse& response, uint8_t* buffer)
{
	uint8_t* ptr = buffer;
...
void ServerLogicClass::destroy(uint8_t* ptr)
{
	if (ptr != nullptr)
	{
		delete[] ptr;
...
void ServerLogicClass::destroy(SRequest* request)
{
	if (request != nullptr)
	{
...
void ServerLogicClass::destroy(SResponse* response)
{
	if (response != nullptr)
	{
...
bool ServerLogicClass::lock(const SRequest& request)
{
...
void ServerLogicClass::unlock(const SRequest& request)
{
	if (request.header.userId == 0)
		return;
